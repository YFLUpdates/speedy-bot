...
function missing (user){
    const request = axios.get(`https://xayo.pl/${user}`, {
        ...
        if(data){
            return `${Censor(user)} ostatnio był(a) widzany(a) u ${data.streamer.login} (${new Date(data.timestamp).toLocaleString("pl")}) oho`;
        }else{
            return `${Censor(user)} nie był(a) nigdzie widziany(a) aha`;
        }
